...
function updateData($nameFile, $WAver, $classesMD5 = "", $memcache = null)
{
    ...
    if ($memcache)
    {
        switch ($nameFile) {
            case 'token.php':
                $memcache->set('classes_md5', trim($classesMD5));
            break;

            case 'whatsprot.class.php':
                $memcache->set('whatsapp_ver', trim($WAver));
                $memcache->set('whatsapp_user_agent', 'WhatsApp/' . trim($WAver) . ' Android/4.3 Device/GalaxyS3');
            break;
        }
    } else {
        ...
        if ($file == __DIR__ . '/token.php') {
            $content[10] = '    $classesMd5 = ' . "\"" . trim($classesMD5) . "\"; // $WAver";
            $content[13] = '    $classesMd5 = ' . "\"" . trim($classesMD5) . "\"; // $WAver";
        } else {
            if ($file == __DIR__ . '/whatsprot.class.php') {
                $content[86] = '    const WHATSAPP_VER = \'' . trim($WAver) . '\';                // The WhatsApp version.';
                $content[87] = '    const WHATSAPP_USER_AGENT = \'WhatsApp/' . trim($WAver) . ' Android/4.3 Device/GalaxyS3\'; // User agent used in request/registration code.';
            }
        }
